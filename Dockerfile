...
RUN patchzips=`ls /u01/patches/*p*.zip 2>/dev/null`; \
    if [ ! -z "$patchzips" ]; then \
      ...
      echo -e ""; \
	  echo -e "\nUnzip patches"; \
	  $JAVA_HOME/bin/jar xf 2.p32253037_122140_Generic.zip; \
	  $JAVA_HOME/bin/jar xf 3.p31544353_122140_Linux-x86-64.zip; \
	  $JAVA_HOME/bin/jar xf 4.p32337168_122140_Generic.zip; \
	  $JAVA_HOME/bin/jar xf 5.p32121987_122140_Generic.zip; \
	  $JAVA_HOME/bin/jar xf 6.p32124456_122140_Generic.zip; \
	  rm -f /u01/patches/*p*.zip; \
	  echo -e "\nApply patches"; \
	  cd /u01/patches/32253037; \
	  $ORACLE_HOME/OPatch/opatch apply -silent -oh $ORACLE_HOME -jre $JAVA_HOME -invPtrLoc /u01/oraInst.loc; \
	  cd /u01/patches/31544353; \
	  $ORACLE_HOME/OPatch/opatch apply -silent -oh $ORACLE_HOME -jre $JAVA_HOME -invPtrLoc /u01/oraInst.loc; \
	  cd /u01/patches/32337168; \
	  $ORACLE_HOME/OPatch/opatch apply -silent -oh $ORACLE_HOME -jre $JAVA_HOME -invPtrLoc /u01/oraInst.loc; \
	  cd /u01/patches/32121987; \
	  $ORACLE_HOME/OPatch/opatch apply -silent -oh $ORACLE_HOME -jre $JAVA_HOME -invPtrLoc /u01/oraInst.loc; \
	  cd /u01/patches/122147; \
	  $ORACLE_HOME/OPatch/opatch apply -silent -oh $ORACLE_HOME -jre $JAVA_HOME -invPtrLoc /u01/oraInst.loc; \
